...
export default function TarjetaCentro({ centro, seleccionado, onClick }: Props) {
  ...
  const abierto = centro.activo && nivel !== 'expirado'
      ...
      <div className="px-4 py-3">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <p className="font-bold text-gray-900 text-sm truncate">{centro.nombre}</p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {centro.fecha_fin && nivel !== 'normal' && nivel !== 'expirado' && (
        ...
        {preview.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2.5 ml-4">
            {preview.map(item => (
              <span key={item} className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">
                {item}
              </span>
            ))}
            {extra > 0 && (
